#include "divwindow.h"
DivWindow::DivWindow(QWidget *parent) : QMainWindow(parent)
{
    rank=1;
    flag=1;
...
void DivWindow::paintEvent(QPaintEvent *)
{
    QPixmap back_icon;
...
void DivWindow::mouseMoveEvent(QMouseEvent *event)
{
    QPoint EndPos;
...
void DivWindow::mousePressEvent(QMouseEvent *event)
{
    if(event->buttons()==Qt::LeftButton)
    {
...
void DivWindow::getRandomNumber()
{
    ...
    if(rank==1)
    number1=(qrand()%9)*number2;
    else if(rank==2)
    number1=(qrand()%99)*number2;
    else
    number1=(qrand()%999)*number2;
}
void DivWindow::levelup()
{
    rank++;
}
