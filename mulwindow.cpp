...
Mulwindow::Mulwindow(QWidget *parent) : QMainWindow(parent)
{
    rank=1;
    flag=1;
...
void Mulwindow::paintEvent(QPaintEvent *)
{
    QPixmap back_icon;
...
void Mulwindow::mouseMoveEvent(QMouseEvent *event)
{
    QPoint EndPos;
...
void Mulwindow::mousePressEvent(QMouseEvent *event)
{
    if(event->buttons()==Qt::LeftButton)
    {
...
bool Mulwindow::eventFilter(QObject *target,QEvent *event)
{
    if(target == this->daan)
    {
        if(event->type()==QEvent::KeyPress)
        {
            ...
            if(k->key()==Qt::Key_Return||k->key()==Qt::Key_Enter)
            {
                ...
                {
                    QString labeltext;
                    labeltext="回答错误,正确答案为"+QString::number(number1*number2,10);
...
void Mulwindow::getRandomNumber()
{
